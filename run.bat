...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if ($t -notmatch 'ExpectedVersion\s*=\s*\"2\.2\.5\"') { throw 'run.ps1 is not v2.2.5 — sync failed' };" ^
  "Write-Host '[BOOT] run.ps1 OK v2.2.5'"

if errorlevel 1 (
  echo [FATAL] Could not download run.ps1 v2.2.5
