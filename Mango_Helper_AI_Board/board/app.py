...
class BoardApp(tk.Tk):
    ...
    def _build_welcome(self, parent: tk.Frame) -> None:
        tk.Label(
            parent,
            text=f"{APP_SHORT_KO} ({APP_SHORT_EN})",
    ...
    def _run_merge_update(self) -> None:
        ...
        msg = (
            f"{APP_SHORT_KO}를 종료한 뒤 GitHub main 을 강제 반영하고 재시작합니다.\n\n"
            f"현재 버전: v{cur}\n\n"
        ...
        self.lbl_update_status.configure(
            text=f"{APP_SHORT_KO} 종료 → 강제 버전갱신 → 재시작",
            fg="#0f172a",
        )
        ...
        messagebox.showinfo(
            "버전갱신 시작",
            f"{APP_SHORT_KO}를 종료합니다.\n\n"
            "이어서 자동으로:\n"
            "1) GitHub main 강제 반영\n"
            f"2) {APP_SHORT_KO} 재시작",
            parent=self,
        )
        self.destroy()
