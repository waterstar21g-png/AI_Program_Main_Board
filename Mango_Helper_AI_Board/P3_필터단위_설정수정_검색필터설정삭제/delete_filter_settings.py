...
사용법:
  python delete_filter_settings.py --row-from 1 --row-to 5
  python delete_filter_settings.py --site-id MUSINSA.com --row-from 11 --row-to 11
  python delete_filter_settings.py --list-rows --row-from 11 --row-to 11   # 확인만
"""
...
STOP_FLAG_PATH = HERE / ".delete_stop"
    ...
    try:
        with sync_playwright() as pw:
            ...
            for i, row in enumerate(rows, start=1):
                ...
                if delete_one_row(page, row, list_url=url, progress=progress):
                    result.deleted += 1
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="P3_필터단위_설정수정_검색필터설정삭제")
