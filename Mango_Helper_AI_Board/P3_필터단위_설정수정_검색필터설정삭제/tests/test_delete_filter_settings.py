"""P3_필터단위_설정수정_검색필터설정삭제 단위테스트 — 브라우저 없이 검증."""
...
import delete_filter_settings as dfs  # noqa: E402
...
def test_click_delete_setting_clicks_button():
    popup = FakePopup()
    logs: list[str] = []
    assert dfs.click_delete_setting(popup, progress=logs.append) is True
...
def test_click_delete_setting_falls_back_to_direct_call():
    ...
    assert dfs.click_delete_setting(popup) is True
...
def test_delete_one_row_full_sequence():
    ...
    ok = dfs.delete_one_row(PopupHost(popup), row, list_url=dfs.DEFAULT_LIST_URL, progress=logs.append)
...
def test_delete_one_row_closes_popup_even_on_failure():
    ...
    ok = dfs.delete_one_row(PopupHost(popup), row, list_url=dfs.DEFAULT_LIST_URL)
    assert ok is False
...
def test_run_delete_processes_row_range(monkeypatch):
    ...
    result = dfs.run_delete(row_from=2, row_to=4)
