...
if (-not $env:AI_BOARD_UPDATER_REFRESHED) {
  ...
  foreach ($url in $selfUrls) {
    try {
      ...
      $wc.Headers.Add("User-Agent", "Mango_Recreate_Board-updater-self")
...
function Get-RemoteVersionViaHttp {
  ...
  foreach ($url in $urls) {
    try {
      $wc = New-Object System.Net.WebClient
      $wc.Headers.Add("User-Agent", "Mango_Recreate_Board-update-if-newer")
      $wc.Headers.Add("Cache-Control", "no-cache")
...
function Update-FromZip {
  ...
  $zipFile = Join-Path $env:TEMP "Mango_Recreate_Board_main_auto.zip"
  $extractDir = Join-Path $env:TEMP ("Mango_Recreate_Board_extract_" + [Guid]::NewGuid().ToString("N"))
  try {
    if (Test-Path -LiteralPath $zipFile) { Remove-Item -LiteralPath $zipFile -Force -ErrorAction SilentlyContinue }
    $wc = New-Object System.Net.WebClient
    $wc.Headers.Add("User-Agent", "Mango_Recreate_Board-zip-fallback")
    $wc.Headers.Add("Cache-Control", "no-cache")
