...
def pull_main(root: Path) -> tuple[bool, str]:
    ...
    # 로컬 수정이 있어도 머지 반영이 우선 (아이콘 체인의 reset 과 유사)
...
def apply_update(root: Path | None = None) -> dict:
    ...
    반환: {
      ok, local_before, local_after, remote, message, method
    }
    """
...
def latest_open_pr_url() -> str:
    """가능하면 최신 open PR URL (머지 URL). 실패 시 저장소 PR 목록."""
    root = root_dir()
    code, out = _run(
        [
            "gh",
...
def restart_board(root: Path | None = None) -> None:
    """새 보드 프로세스를 띄운 뒤 현재 프로세스는 호출측에서 종료."""
    root = root or root_dir()
    ...
    if os.name == "nt":
        # 새 콘솔 없이 보드만 재기동 (CREATE_NO_WINDOW 는 GUI Tk 에 부적합할 수 있어 DETACHED만)
