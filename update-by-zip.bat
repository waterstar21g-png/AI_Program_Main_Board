...
echo ========================================
echo   AI_Program_Main_Board  ZIP 강제 갱신
echo   (git pull / 스크립트 개별 다운로드가
echo    막히는 PC를 위한 대안 — 브라우저에서
echo    "Download ZIP" 받는 것과 동일한 효과)
echo ========================================

set "REPO=waterstar21g-png/AI_Program_Main_Board"
set "ZIP_URL=https://codeload.github.com/%REPO%/zip/refs/heads/main"
set "ZIP_FILE=%TEMP%\AI_Program_Main_Board_main.zip"
set "EXTRACT_DIR=%TEMP%\AI_Program_Main_Board_extract_%RANDOM%"
...
if not exist "%EXTRACT_DIR%\AI_Program_Main_Board-main" (
  echo [ERROR] 압축 해제 실패
  pause
  exit /b 1
)
...
xcopy "%EXTRACT_DIR%\AI_Program_Main_Board-main\*" "%~dp0" /E /H /Y /I >nul
if errorlevel 1 (
  echo [ERROR] 파일 복사 실패
  pause
  exit /b 1
