  ...
  try {
    ...
    checks.push(
      check(
        ...
        labelsOk && urlsOk
          ? `라벨/URL 분리 정상 — 예: ${parsed[0]?.topFinalLabel}`
          : '라벨·URL 필드 검증 실패',
      ),
    );
    ...
    if (process.env.VERCEL) {
      ...
      try {
        ...
        checks.push(check('2) Playwright', 'pass', 'playwright 로드 정상'));
      } catch (e) {
        checks.push(
...
`;
        ...
        checks.push(
          check(
            '3) 데이터 검증 · 샘플엑셀',
            read.ok ? 'pass' : 'fail',
            read.ok ? `openpyxl ${read.out}행 읽기 정상` : read.out.slice(0, 200),
          ),
        );
      } finally {
        try {
    ...
    checks.push(
      check(
        '2) playwright(python)',
        pw.ok ? 'pass' : 'warn',
        pw.ok ? 'import 정상' : '미설치 — pip install -r python-collector/requirements.txt',
      ),
    );
  }
