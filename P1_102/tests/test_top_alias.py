"""P1_102(P1 복제본) 상위 카테고리 칸 파싱 · 엑셀 치환 · 상품수 파싱 단위테스트."""
...
def test_save_excel_filename_tagged_102(tmp_path):
    """P1_102 저장 파일명은 P1과 겹치지 않도록 "_102" 표식이 붙는다."""
    path = save_excel([], "ABC마트", tmp_path)
    assert path.exists()
    assert "_102" in path.name
    assert path.name.startswith("ABC마트_카테고리URL_LIST_102_")
...
if __name__ == "__main__":
    ...
    print("ok (참고: test_save_excel_filename_tagged_102 는 pytest tmp_path 필요 — pytest로 실행)")
