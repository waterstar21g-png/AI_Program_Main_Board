...
모달에 검색필터명·저장상품수(수집상품수) 입력 후
**하단 버튼명 「저장하기」클릭(필수)** = 서버 최종 갱신.
모달 전체(하단 저장하기·취소하기 포함)를 잡고 푸터로 스크롤한 뒤 클릭합니다.
클릭 후 **저장 실행 팝업/알림 모달**이 보여야 하며, 반응 없으면 **최대 1회 재시도** 후 다음 행입니다.
성공 시 망고 **「N건이 수집되었다」** 건수 검증 후 행 완료입니다.
